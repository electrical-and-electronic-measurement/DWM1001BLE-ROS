...
# Decawave characteristics for network node service (from documentation)
...
ConfiguredDevicesMACS = list(map(lambda elm: elm["MAC"], Devices))
    ...
    def __init__(
        ...
        for advertising_data_tuple in advertising_data_tuples:
            ...
            if type_code == SHORT_LOCAL_NAME_TYPE_CODE:
                self.device_name = value.strip('\x00')
...
def scan_for_decawave_devices():
    ...
    for decawave_scan_entry in decawave_scan_entries:
        ...
        decawave_device.device_name = list(
            filter(lambda device: decawave_device.device_name in device["Name"], Devices))[0]["Name"]
...
def get_decawave_scan_entries():
    ...
    # for scan_entrie in list(scan_entries):
    #     print(scan_entrie.getScanData(), "\n")
    #     print(scan_entrie.addr, scan_entrie.iface, "\n")
...
def is_decawave_scan_entry(scan_entry):
    ...
    return short_local_name is not None and scan_entry.addr.upper() in ConfiguredDevicesMACS


# Function for connecting to Decawave device
# @exponential_retry
def get_decawave_peripheral(decawave_device):
    decawave_peripheral = bluepy.btle.Peripheral(deviceAddr=decawave_device.mac_address,
...
def parse_location_data_bytes(location_data_bytes):
    ...
    if (location_data_content == 1 or location_data_content == 2):
        ...
        if len(location_data_bytes) < 7 * distance_count:
            raise ValueError(
                'Distance count byte indicated that {} distance values would follow so expected {} bytes but only {} bytes follow'.format(
                    distance_count,
                    7 * distance_count,
